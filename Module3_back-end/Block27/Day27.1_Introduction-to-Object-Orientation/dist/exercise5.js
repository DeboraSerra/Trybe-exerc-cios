...
class MyDate {
    ...
    ;
    set day(day) { if (day > 0 && day < 31)
        this._day = day; }
    ;
    set month(month) { if (month > 0 && month < 12)
        this._month = month; }
    ;
    set year(year) { if (year > 1900)
        this._year = year; }
    ;
    get monthName() {
    ...
    compare(date) {
        if (new Date(date.date) > new Date(this.date))
            return -1;
        if (new Date(date.date) > new Date(this.date))
            return 1;
        return 0;
    ...
    format(format) {
        ...
        if (format.includes(','))
            return `${this.day}, ${this.monthName} de ${this.year}`;
    }
}
